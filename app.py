...
def process_uploaded_file(uploaded_file):
    ...
    with st.spinner("Loading document..."):
        ...
        image_docs = []
        if image_paths:
            with st.spinner(f"Analyzing {len(image_paths)} images..."):
    ...
    if st.session_state.quiz_questions is None:
        if st.button("Generate 10 Questions", type="primary"):
            with st.spinner("Generating quiz questions..."):
                # Use FULL TEXT context for better coverage (up to limit)
                # This ensures the quiz covers the entire document, not just samples
                
                full_text = "\n\n".join([chunk.page_content for chunk in st.session_state.all_chunks])
                
                # Limit to ~100k chars to avoid token limits (approx 25k tokens)
                if len(full_text) > 100000:
                    st.warning("Document is very large. Using the first ~100,000 characters for quiz generation.")
                ...
                # Detect main topic first
                main_topic = quiz_gen.detect_main_topic(context)
                st.info(f"Quiz Topic: **{main_topic}**")
                
                questions = quiz_gen.generate_quiz(context, num_questions=10, topic=main_topic)
...
def main():
    ...
    if st.session_state.documents_loaded:
        ...
        - **Image Analysis**: Extracts text from diagrams, charts, and formulas
